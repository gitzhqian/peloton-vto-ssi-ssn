...
storage::DataTable* region_table;
storage::DataTable* nation_table;
storage::DataTable* supplier_table;
...
//                      "W_ID", is_inlined);
  auto w_id_column =
      catalog::Column(type::Type::VARCHAR, 8,
                      "W_ID", is_inlined);
...
//      type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER), "D_ID", is_inlined);
  auto d_id_column = catalog::Column(type::Type::VARCHAR, 8, "D_ID", is_inlined);
  district_columns.push_back(d_id_column);
//  auto d_w_id_column =
//      catalog::Column(type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER),
//                      "D_W_ID", is_inlined);
  auto d_w_id_column = catalog::Column(type::Type::VARCHAR, 8,"D_W_ID", is_inlined);
...
//      type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER), "I_ID", is_inlined);
  auto i_id_column = catalog::Column(type::Type::VARCHAR, 8, "I_ID", is_inlined);
...
//      type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER), "C_ID", is_inlined);
  auto c_id_column = catalog::Column(
      type::Type::VARCHAR, 8, "C_ID", is_inlined);
...
//      type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER), "O_ID", is_inlined);
  auto o_id_column = catalog::Column(
      type::Type::VARCHAR, 8, "O_ID", is_inlined);
...
void CreateNationTable() {
   ...
   */
  // Create schema first
  std::vector<catalog::Column> nation_columns;

//  auto n_nationkey_column =
//      catalog::Column(type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER),
//                      "N_NATIONKEY", is_inlined);
  auto n_nationkey_column =
      catalog::Column(type::Type::VARCHAR, 8,
                      "N_NATIONKEY", is_inlined);
  nation_columns.push_back(n_nationkey_column);
  auto n_regionkey_column =
      catalog::Column(type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER),
                      "N_REGIONKEY", is_inlined);
  nation_columns.push_back(n_regionkey_column);
  auto n_name_column =
      catalog::Column(type::Type::VARCHAR, name_length,
                      "N_NAME", is_inlined);
  nation_columns.push_back(n_name_column);
  auto n_comment_column =
      catalog::Column(type::Type::VARCHAR, data_length,
                      "N_COMMENT", is_inlined);
  nation_columns.push_back(n_comment_column);

  catalog::Schema *table_schema = new catalog::Schema(nation_columns);
  std::string table_name("NATION");

  nation_table = storage::TableFactory::GetDataTable(
      tpcc_database_oid, nation_table_oid, table_schema, table_name,
      DEFAULT_TUPLES_PER_TILEGROUP, own_schema, adapt_table);

  tpcc_database->AddTable(nation_table);

  auto tuple_schema = nation_table->GetSchema();
  std::vector<oid_t> key_attrs;
  catalog::Schema *key_schema = nullptr;
  index::IndexMetadata *index_metadata = nullptr;

  // Primary index on n_nationkey
  key_attrs = {0};
  key_schema = catalog::Schema::CopySchema(tuple_schema, key_attrs);
  key_schema->SetIndexedColumns(key_attrs);

  index_metadata = new index::IndexMetadata(
      "nation_pkey", nation_table_pkey_index_oid, nation_table_oid,
      tpcc_database_oid, state.index, IndexConstraintType::PRIMARY_KEY,
      tuple_schema, key_schema, key_attrs, true);
  ...
  std::shared_ptr<index::Index> pkey_index(
      index::IndexFactory::GetIndex(index_metadata));
  nation_table->AddIndex(pkey_index);
}
void CreateRegionTable() {
   ...
   */
  // Create schema first
  std::vector<catalog::Column> region_columns;

//  auto r_regionkey_column =
//      catalog::Column(type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER),
//                      "R_REGIONKEY", is_inlined);
  auto r_regionkey_column =
      catalog::Column(type::Type::VARCHAR, 8,
                      "R_REGIONKEY", is_inlined);
  region_columns.push_back(r_regionkey_column);
  auto r_name_column =
      catalog::Column(type::Type::VARCHAR, name_length,
                      "R_NAME", is_inlined);
  region_columns.push_back(r_name_column);
  auto r_comment_column =
      catalog::Column(type::Type::VARCHAR, data_length,
                      "R_COMMENT", is_inlined);
  region_columns.push_back(r_comment_column);


  catalog::Schema *table_schema = new catalog::Schema(region_columns);
  std::string table_name("REGION");

  region_table = storage::TableFactory::GetDataTable(
      tpcc_database_oid, region_table_oid, table_schema, table_name,
      DEFAULT_TUPLES_PER_TILEGROUP, own_schema, adapt_table);

  tpcc_database->AddTable(region_table);

  auto tuple_schema = region_table->GetSchema();
  std::vector<oid_t> key_attrs;
  catalog::Schema *key_schema = nullptr;
  index::IndexMetadata *index_metadata = nullptr;

  // Primary index on r_regionkey
  key_attrs = {0};
  key_schema = catalog::Schema::CopySchema(tuple_schema, key_attrs);
  key_schema->SetIndexedColumns(key_attrs);

  index_metadata = new index::IndexMetadata(
      "region_pkey", region_table_pkey_index_oid, region_table_oid,
      tpcc_database_oid, state.index, IndexConstraintType::PRIMARY_KEY,
      tuple_schema, key_schema, key_attrs, true);
  ...
  std::shared_ptr<index::Index> pkey_index(
      index::IndexFactory::GetIndex(index_metadata));
  region_table->AddIndex(pkey_index);
...
void CreateSupplierTable() {
   ...
   */
  std::vector<catalog::Column> supplier_columns;

//  auto su_suppkey_column =
//      catalog::Column(type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER),
//                      "SU_SUPPKEY", is_inlined);
  auto su_suppkey_column =
      catalog::Column(type::Type::VARCHAR, 8,
                      "SU_SUPPKEY", is_inlined);
  supplier_columns.push_back(su_suppkey_column);
  auto su_nationkey_column =
      catalog::Column(type::Type::INTEGER, type::Type::GetTypeSize(type::Type::INTEGER),
                      "SU_NATIONKEY", is_inlined);
  supplier_columns.push_back(su_nationkey_column);
  auto su_acctbal_column =
      catalog::Column(type::Type::DECIMAL, type::Type::GetTypeSize(type::Type::DECIMAL),
                      "SU_ACCTBAL", is_inlined);
  supplier_columns.push_back(su_acctbal_column);
  auto su_name_column =
      catalog::Column(type::Type::VARCHAR, name_length,
                      "SU_NAME", is_inlined);
  supplier_columns.push_back(su_name_column);
  auto su_address_column =
      catalog::Column(type::Type::VARCHAR, street_length,
                      "SU_ADDRESS", is_inlined);
  supplier_columns.push_back(su_address_column);
  auto su_phone_column =
      catalog::Column(type::Type::VARCHAR, phone_length,
                      "SU_PHONE", is_inlined);
  supplier_columns.push_back(su_phone_column);
  auto su_comment_column =
      catalog::Column(type::Type::VARCHAR, data_length,
                      "SU_COMMENT", is_inlined);
  supplier_columns.push_back(su_comment_column);

  catalog::Schema *table_schema = new catalog::Schema(supplier_columns);
  std::string table_name("SUPPLIER");

  supplier_table = storage::TableFactory::GetDataTable(
      tpcc_database_oid, supplier_table_oid, table_schema, table_name,
      DEFAULT_TUPLES_PER_TILEGROUP, own_schema, adapt_table);

  tpcc_database->AddTable(supplier_table);

  auto tuple_schema = supplier_table->GetSchema();
  std::vector<oid_t> key_attrs;
  catalog::Schema *key_schema = nullptr;
  index::IndexMetadata *index_metadata = nullptr;

  // Primary index on su_suppkey
  key_attrs = {0};
  key_schema = catalog::Schema::CopySchema(tuple_schema, key_attrs);
  key_schema->SetIndexedColumns(key_attrs);

  index_metadata = new index::IndexMetadata(
      "supplier_pkey", supplier_table_pkey_index_oid, supplier_table_oid,
      tpcc_database_oid, state.index, IndexConstraintType::PRIMARY_KEY,
      tuple_schema, key_schema, key_attrs, true);
  ...
  std::shared_ptr<index::Index> pkey_index(
      index::IndexFactory::GetIndex(index_metadata));
  supplier_table->AddIndex(pkey_index);
}
...
//  item_tuple->SetValue(0, type::ValueFactory::GetIntegerValue(item_id), nullptr);
  item_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(item_id)), pool.get());
  // I_IM_ID
...
//                            nullptr);
  warehouse_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(warehouse_id)),
                            pool.get());
  // W_NAME
...
//                           nullptr);
  district_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(district_id)),
                           pool.get());
  // D_W_ID
//  district_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(warehouse_id),
//                           nullptr);
  district_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(std::to_string(warehouse_id)),
                           pool.get());
  // D_NAME
...
//                           nullptr);
  customer_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(customer_id)),
                           pool.get());
  // C_D_ID
//  customer_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(district_id),
//                           nullptr);
  customer_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(std::to_string(district_id)),
                           pool.get());
  // C_W_ID
//  customer_tuple->SetValue(2, type::ValueFactory::GetIntegerValue(warehouse_id),
//                           nullptr);
  customer_tuple->SetValue(2, type::ValueFactory::GetVarcharValue(std::to_string(warehouse_id)),
                           pool.get());
...
//  orders_tuple->SetValue(0, type::ValueFactory::GetIntegerValue(orders_id), nullptr);
  orders_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(orders_id)), pool.get());
  // O_C_ID
...
//  orders_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(o_c_id), nullptr);
  orders_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(std::to_string(o_c_id)), pool.get());
  // O_D_ID
//  orders_tuple->SetValue(2, type::ValueFactory::GetIntegerValue(district_id),
//                         nullptr);
  orders_tuple->SetValue(2, type::ValueFactory::GetVarcharValue(std::to_string(district_id)),
                         pool.get());
  // O_W_ID
//  orders_tuple->SetValue(3, type::ValueFactory::GetIntegerValue(warehouse_id),
//                         nullptr);
  orders_tuple->SetValue(3, type::ValueFactory::GetVarcharValue(std::to_string(warehouse_id)),
                         pool.get());
  // O_ENTRY_D
...
//                            nullptr);
  new_order_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(orders_id)),
                            pool.get());
  // NO_D_ID
//  new_order_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(district_id),
//                            nullptr);
  new_order_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(std::to_string(district_id)),
                            pool.get());
  // NO_W_ID
//  new_order_tuple->SetValue(2, type::ValueFactory::GetIntegerValue(warehouse_id),
//                            nullptr);
  new_order_tuple->SetValue(2, type::ValueFactory::GetVarcharValue(std::to_string(warehouse_id)),
                            pool.get());
...
//                             nullptr);
  order_line_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(orders_id)),
                             pool.get());
  // OL_D_ID
//  order_line_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(district_id),
//                             nullptr);
  order_line_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(std::to_string(district_id)),
                             pool.get());
  // OL_W_ID
//  order_line_tuple->SetValue(2, type::ValueFactory::GetIntegerValue(warehouse_id),
//                             nullptr);
  order_line_tuple->SetValue(2, type::ValueFactory::GetVarcharValue(std::to_string(warehouse_id)),
                             pool.get());
  // OL_NUMBER
//  order_line_tuple->SetValue(3, type::ValueFactory::GetIntegerValue(order_line_id),
//                             nullptr);
  order_line_tuple->SetValue(3, type::ValueFactory::GetVarcharValue(std::to_string(order_line_id)),
                             pool.get());
  // OL_I_ID
...
//  stock_tuple->SetValue(0, type::ValueFactory::GetIntegerValue(stock_id), nullptr);
  stock_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(stock_id)), pool.get());
  // S_W_ID
//  stock_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(s_w_id), nullptr);
  stock_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(std::to_string(s_w_id)), pool.get());
  // S_QUANTITY
...
std::unique_ptr<storage::Tuple>  BuildRegionTuple(const int region_id,
                                                  const std::unique_ptr<type::AbstractPool> &pool){
//  int64_t R_REGIONKEY;
//  char R_NAME[55];
//  char R_COMMENT[152];
  auto region_table_schema = region_table->GetSchema();
  std::unique_ptr<storage::Tuple> region_tuple(
      new storage::Tuple(region_table_schema, allocate));

  // R_REGIONKEY
//  region_tuple->SetValue(0, type::ValueFactory::GetIntegerValue(region_id), nullptr);
  region_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(region_id)), pool.get());
  // R_NAME
  auto r_name = std::string(regions[region_id]);
  region_tuple->SetValue(1, type::ValueFactory::GetVarcharValue(r_name), pool.get());
  // R_COMMENT
  auto r_comment = GetRandomAlphaNumericString(data_length);
  region_tuple->SetValue(2, type::ValueFactory::GetVarcharValue(r_comment), pool.get());

  return region_tuple;
}
std::unique_ptr<storage::Tuple> BuildNationTuple(const int nation_id,
                        const std::unique_ptr<type::AbstractPool> &pool){
//  int64_t N_NATIONKEY;
//  int64_t N_REGIONKEY;
//  char N_NAME[25];
//  char N_COMMENT[152];
  auto nation_table_schema = nation_table->GetSchema();
  std::unique_ptr<storage::Tuple> nation_tuple(
      new storage::Tuple(nation_table_schema, allocate));

  // N_NATIONKEY
//  nation_tuple->SetValue(0, type::ValueFactory::GetIntegerValue(nation_id), nullptr);
  nation_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(nation_id)), pool.get());
  // N_REGIONKEY
  nation_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(nations[nation_id].N_REGIONKEY), nullptr);
  // N_NAME
  auto n_name = nations[nation_id].N_NAME;
  nation_tuple->SetValue(2, type::ValueFactory::GetVarcharValue(n_name), pool.get());
  // N_COMMENT
  auto n_comment = GetRandomAlphaNumericString(data_length);
  nation_tuple->SetValue(3, type::ValueFactory::GetVarcharValue(n_comment), pool.get());

  return nation_tuple;
}
std::unique_ptr<storage::Tuple> BuildSupplierTuple(const int supp_id,
                                                   const std::unique_ptr<type::AbstractPool> &pool){
//  int64_t SU_SUPPKEY;
//  int64_t SU_NATIONKEY;
//  double SU_ACCTBAL[8]; //double
//  char SU_NAME[25];
//  char SU_ADDRESS[40];
//  char SU_PHONE[15];
//  char SU_COMMENT[15];
  auto supplier_table_schema = supplier_table->GetSchema();
  std::unique_ptr<storage::Tuple> supplier_tuple(
      new storage::Tuple(supplier_table_schema, allocate));

  // SU_SUPPKEY
//  supplier_tuple->SetValue(0, type::ValueFactory::GetIntegerValue(supp_id), nullptr);
  supplier_tuple->SetValue(0, type::ValueFactory::GetVarcharValue(std::to_string(supp_id)), pool.get());
  //nation key
  auto nation_k = GetRandomInteger(0, 61);
  supplier_tuple->SetValue(1, type::ValueFactory::GetIntegerValue(nation_k), nullptr);
  //acctbal
  //has nobody use it
  auto accbal_ = GetRandomDouble(warehouse_min_tax, warehouse_max_tax);
  supplier_tuple->SetValue(2, type::ValueFactory::GetDecimalValue(accbal_), nullptr);
  //name
  std::string name_ = std::string("Supplier#") + std::string("000000000") + std::to_string(supp_id);
  supplier_tuple->SetValue(3, type::ValueFactory::GetVarcharValue(name_), pool.get());
  //address
  auto street_ = GetRandomAlphaNumericString(street_length);
  supplier_tuple->SetValue(4, type::ValueFactory::GetVarcharValue(street_), pool.get());
  //phone
  auto phone_ = GetRandomAlphaNumericString(phone_length);
  supplier_tuple->SetValue(5, type::ValueFactory::GetVarcharValue(phone_), pool.get());
  //comment
  auto comment_ = GetRandomAlphaNumericString(data_length);
  supplier_tuple->SetValue(6, type::ValueFactory::GetVarcharValue(comment_), pool.get());
...
void LoadNation() {
  ...
  for (uint n_itr = 0; n_itr < 62; n_itr++) {
    auto nation_tuple = BuildNationTuple(n_itr, pool);
    planner::InsertPlan node(nation_table, std::move(nation_tuple));
...
void LoadRegion() {
  ...
  for (auto reg_itr = 0; reg_itr < 5; reg_itr++) {
    auto reg_tuple = BuildRegionTuple(reg_itr, pool);
    planner::InsertPlan node(region_table, std::move(reg_tuple));
    executor::InsertExecutor executor(&node, context.get());
    executor.Execute();
  }
...
void LoadSupplier() {
  ...
  for (uint supp_i = 0; supp_i < 10000; supp_i++) {
    auto supp_tuple = BuildSupplierTuple(supp_i, pool);
    planner::InsertPlan node(supplier_table, std::move(supp_tuple));
    executor::InsertExecutor executor(&node, context.get());
    executor.Execute();
  }
    ...
    for (auto district_itr = 0; district_itr < state.districts_per_warehouse;
      ...
      for (auto orders_itr = 0; orders_itr < state.customers_per_district;
        ...
        auto orders_tuple = BuildOrdersTuple(
            orders_itr, district_itr, warehouse_itr, new_order, o_ol_cnt, pool);
        ...
        if (new_order) {
          auto new_order_tuple =
              BuildNewOrderTuple(orders_itr, district_itr, warehouse_itr, pool);
