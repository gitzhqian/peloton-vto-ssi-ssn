...
void ParseArguments(int argc, char *argv[], configuration &state) {
  ...
  state.concurrency_type = 0;//0-time/1-ssi/2-ssn


  // Parse args
  while (1) {
    int idx = 0;
    int c = getopt_long(argc, argv, "heagi:k:d:p:b:w:n:l:y:s:c:o:t:W:", opts, &idx);
    ...
    switch (c) {
      ...
      case 'l':
        ...
        break;
      case 'c':
        state.concurrency_type = atoi(optarg);
        break;
      case 's':
...
void WriteOutput() {
  ...
  LOG_INFO("----------------------------------------------------------");
  LOG_INFO("%lf %d %d :: %lf %lf %lf %lf %lf %d",
           state.scale_factor,
           state.backend_count,
  ...
  for (size_t round_id = 0; round_id < state.profile_throughput.size();++round_id) {
