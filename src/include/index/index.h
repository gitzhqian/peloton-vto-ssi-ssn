...
class Index : public Printable {
 ...
 public:
  ...
  virtual void ScanLimit(const std::vector<type::Value> &value_list,
                         ...
                         uint64_t offset) = 0;
  virtual void ScanLimitRange(
      const std::vector<type::Value> &value_list,
      const std::vector<oid_t> &tuple_column_id_list,
      const std::vector<ExpressionType> &expr_list,
      ScanDirectionType scan_direction, std::vector<ItemPointer *> &result,
      const ConjunctionScanPredicate *csp_p, uint64_t limit, uint64_t offset) = 0;
