...
bool IndexScanExecutor::ExecPrimaryIndexLookup() {
  ...
  if (0 == key_column_ids_.size()) {
    ...
    if (limit_) {
      if(limit_offset_ < 0 ){
//          LOG_DEBUG("ASCENDING SCAN LIMIT Range in Primary Index");
        index_->ScanLimitRange(values_, key_column_ids_, expr_types_,
        ...
        LOG_TRACE("Scan Limit Result tiles : %lu", result_.size());
  ...
  for (auto tuple_location_ptr : tuple_location_ptrs) {
    ...
    if(concurrency_protocol == ConcurrencyType::CONCURRENCY_TYPE_SSI ||
        concurrency_protocol == ConcurrencyType::CONCURRENCY_TYPE_SI_SSN){
      while (true){
        ++chain_length;

        // if the tuple is visible.
        if (transaction_manager.IsVisible(current_txn, tile_group_header,
                            tuple_location.offset) == VisibilityType::OK) {
...
//            garbage_tuples.push_back(old_item);
      ...
      while (true) {
        ...
        if (visibility == VisibilityType::DELETED) {
          ...
          bool is_alive = (tile_group_header->GetEndCommitId(tuple_location.offset) <=
